// All LLM system prompts. One name per prompt — no inline strings in logic code.
...
/// Entity node extraction prompt. `entity_types` is a comma-separated list from SourceProfiler.
...
/// Edge extraction prompt. `node_ids` and `rel_types` are comma-separated lists from prior pipeline stages.
